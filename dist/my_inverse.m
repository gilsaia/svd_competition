...
function [U,S] = dk_svd_withoutv(B,U,n)
    % B=U*S*V'
...
function [U,B] = twoelementQR(B,U,st)
    f=B(1,1);
    h=B(2,2);
    g=B(1,2);
    ft=B(1,1);
...
function [U,B] = implicitQR(B,U,st)
    n=size(B,1);
    for i=1:n-1
...
function [U,B] = standardQR(B,U,st)
    n=size(B,1);
    a=B(n-1,n-1);
    b=B(n,n);
    c=B(n-1,n);
